    ...
    <div className={`w-full grid grid-cols-1 md:grid-cols-2 ${gridConfig[gridCols]} gap-8 mt-5`}>
      {displayData.map((stat, index) => {
        ...
        return (
          <div
            key={index}
            className="w-full min-h-[140px] border border-gray-200 dark:border-slate-700 rounded-[10px] flex justify-between p-6 bg-white dark:bg-slate-800 transition-all duration-300 group"
          >
            <div className="flex flex-col justify-center">
              ...
              <p className={`text-[15px] mt-1 ${stat.trend ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                }`}>
                {stat.trend}
              </p>
            </div>
